...
Daftar lengkap batasan The Evaluator. Penomoran butir **tidak berubah** dari README versi sebelumnya —
`demo/video-script.md`, `demo/video-script.en.md`, dan `docs/posts/` merujuk butir-butir ini dengan nomor
yang sama. Ringkasan delapan butir terpenting ada di [`README.md`](../README.md).

Butir 1-13 dipindahkan apa adanya dari ADR dan artefak deploy; butir 14-25 ditambahkan sesudah audit "klaim
vs kenyataan" dan sumbernya adalah kode serta chain yang bisa Anda buka sendiri. Angkanya tidak dilunakkan.
...
adalah temuan review atas `web/` yang **belum selesai ditutup** pada saat dokumen ini ditulis.
...
baca butir 22 lebih dulu supaya Anda tahu nilai mana yang seharusnya cocok.
...
yang memilih pembagi 4. Baca kolom "Yang berubah di memori" (`docs/evidence.md`) dengan batas itu.
...
3. file DB **hilang** — **DAN** `lastMemoryRoot()` on-chain **bukan nol**. Kalau root on-chain nol
   ...
   Syarat kedua inilah yang membuat tes destruktif bisa berjalan sama sekali.
...
Ini pembalikan terbesar dalam repo ini.
...
Di explorer mana pun, transaksi, event, dan hasil `cast call` yang dijadikan bukti tetap
...
klaim "md5 identik sebelum/sesudah" pada tabel tes destruktif (`docs/reproduce.md`). Jadi tabel tes
destruktif itu tetap **laporan** yang tidak bisa Anda cocokkan dengan berkas mana pun di repo; yang bisa
Anda lakukan adalah menjalankan `make demo` sendiri dan membandingkannya dengan mata.
...
ulang** seluruh bukti on-chain (`docs/evidence.md`) dengan `cast`/BaseScan, dan (b) mendeploy vault sendiri
dari HEAD — yang bytecode-nya sudah berbeda dari yang terdeploy (butir 15). Reproduksi mandiri "dari nol
sampai tx yang sama" bukan sesuatu yang bisa kami tawarkan.
...
Kesimpulannya, dan ini yang tidak boleh salah dibaca dari diagram alur mana pun:
